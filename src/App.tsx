...
export default function App() {
  ...
  if (!isAuthReady) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-black gap-4">
        <div className="w-12 h-12 border-2 border-zinc-800 border-t-red-600 rounded-full animate-spin" />
  ...
  return (
    <div className="min-h-screen bg-zinc-950 crt-flicker relative selection:bg-red-500 selection:text-white">
      ...
      <header className="border-b border-zinc-800 bg-black p-4 flex flex-col md:flex-row items-center justify-between gap-4 sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <div className="bg-red-600 p-1.5 shadow-[0_0_15px_rgba(220,38,38,0.5)]">
            <Mic2 className="w-6 h-6 text-white" />
          </div>
          <div>
            <h1 className="text-xl font-extrabold tracking-tighter uppercase italic leading-none">
              Gzone <span className="text-red-500">Rap Battle Live Event Voting</span>
            </h1>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="w-1.5 h-1.5 rounded-full bg-red-600 animate-pulse"></span>
              <span className="mono text-[10px] uppercase text-zinc-500 tracking-widest font-bold">Arena Protocol v1.0</span>
            </div>
          </div>
        </div>
...
function VoterView({ battles, votedBattles, user, authError, onLogin }: { battles: Battle[], votedBattles: Record<string, 'A' | 'B'>, user: any, authError: string | null, onLogin: () => void }) {
  ...
  if (authError && !user) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center p-12 text-center">
        <h2 className="text-xl font-black uppercase text-red-500 mb-4 italic">Identity Required</h2>
        ...
        <button 
          onClick={onLogin}
          className="bg-red-600 text-white px-8 py-4 font-black uppercase italic skew-btn-left hover:bg-red-500 flex items-center gap-3"
        >
  ...
  if (!activeBattle) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center p-12 text-center">
        <div className="w-16 h-16 border-4 border-zinc-800 border-t-red-600 rounded-full animate-spin mb-8" />
        <h2 className="text-2xl font-black uppercase opacity-50 italic">No battles in progress</h2>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px_1fr] min-h-[calc(100vh-64px)] w-full">
      {/* Contender A */}
      <div className={`voter-card flex flex-col p-8 justify-between relative group ${userVote === 'A' ? 'active-glow-red bg-red-950/20' : userVote ? 'opacity-30' : ''}`}>
        <div className="space-y-1 relative z-10">
          <span className="mono text-[10px] uppercase tracking-[0.3em] text-red-500 font-black">Arena Slot A</span>
          <h2 className="text-5xl md:text-8xl font-black uppercase italic leading-none tracking-tighter drop-shadow-[0_0_15px_rgba(239,68,68,0.3)]">
        ...
        <div className="relative flex-1 flex items-center justify-center pointer-events-none">
          ...
          >
            <div className="text-7xl md:text-9xl font-black text-red-500 mb-4 tracking-tighter">
            ...
            <div className="stat-bar w-48 md:w-64 mx-auto">
              <motion.div 
                ...
                className="h-full bg-red-600 shadow-[0_0_15px_#ef4444]"
              />
            </div>
            <div className="mt-4 mono text-[10px] text-zinc-500 font-bold uppercase tracking-widest">
              {activeBattle.votesA} UNIT CONFIRMATIONS
            </div>
          </motion.div>
        </div>
        ...
        <button
          ...
          className={`w-full py-6 font-black text-xl md:text-2xl uppercase italic skew-btn-left transition-all z-10
            ${userVote === 'A' ? 'bg-red-600 text-white shadow-[0_0_20px_#ef4444]' : 
      ...
      <div className="bg-zinc-950/50 border-x border-zinc-800 flex flex-col p-6 text-center space-y-12">
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-center gap-2 mt-4">
            ...
            <span className="mono text-[10px] uppercase text-red-500 font-black tracking-widest">LIVE DATA FEED</span>
          </div>
        </div>
      ...
      {/* Contender B */}
      <div className={`voter-card flex flex-col p-8 justify-between relative group text-right ${userVote === 'B' ? 'active-glow-blue bg-blue-950/20' : userVote ? 'opacity-30' : ''}`}>
        <div className="space-y-1 relative z-10">
          <span className="mono text-[10px] uppercase tracking-[0.3em] text-blue-500 font-black">Arena Slot B</span>
          <h2 className="text-5xl md:text-8xl font-black uppercase italic leading-none tracking-tighter drop-shadow-[0_0_15px_rgba(59,130,246,0.3)]">
        ...
        <div className="relative flex-1 flex items-center justify-center pointer-events-none">
          ...
          <motion.div 
             ...
             className="text-center z-10"
          >
            <div className="text-7xl md:text-9xl font-black text-blue-500 mb-4 tracking-tighter">
            ...
            <div className="mt-4 mono text-[10px] text-zinc-500 font-bold uppercase tracking-widest">
                {activeBattle.votesB} UNIT CONFIRMATIONS
            </div>
          </motion.div>
        </div>
        ...
        <button
          ...
          className={`w-full py-6 font-black text-xl md:text-2xl uppercase italic skew-btn-right transition-all z-10
            ${userVote === 'B' ? 'bg-blue-600 text-white shadow-[0_0_20px_#3b82f6]' : 
              userVote ? 'bg-zinc-800 text-zinc-500 cursor-not-allowed opacity-50' : 
...
function AdminView({ battles, user, admins, onLogin, onLogout }: { battles: Battle[], user: any, admins: string[], onLogin: () => void, onLogout: () => void }) {
  ...
  const resetVotes = async (id: string) => {
    if (!confirm("Are you sure you want to reset all votes for this battle? This cannot be undone.")) return;
    try {
      await updateDoc(doc(db, 'battles', id), { 
        votesA: 0, 
        votesB: 0,
    ...
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center text-center">
        <div className="bg-zinc-900 border border-zinc-800 p-12 shadow-2xl">
          ...
          <button 
            onClick={onLogin}
            className="w-full bg-red-600 text-white px-8 py-4 font-black uppercase italic skew-btn-left hover:bg-red-500 transition-all flex items-center justify-center gap-3"
          >
            <LogIn size={20} /> Admin Authorization
          </button>
        </div>
      </div>
    )
  ...
  if (!isAdmin) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center text-center">
        <div className="bg-red-950/20 border-2 border-red-900 p-12 max-w-xl">
           <h2 className="text-3xl font-black uppercase italic text-red-500 mb-4">Unauthorized Internal</h2>
           ...
           <button onClick={onLogout} className="text-xs uppercase underline opacity-50 hover:opacity-100">De-Authorize</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center p-4 bg-zinc-900 border border-zinc-800 border-l-red-600 border-l-4">
        ...
        <div className="flex gap-4">
          <button 
            ...
            className={`text-[10px] uppercase font-bold flex items-center gap-1 px-3 py-1 border transition-all ${showAdminPanel ? 'bg-red-600 text-white border-red-500' : 'text-zinc-500 border-zinc-800 hover:text-white'}`}
          >
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-8 space-y-6">
            ...
            <form onSubmit={addAdmin} className="flex gap-2">
                <input 
                    ...
                    className="flex-1 bg-black border border-zinc-800 p-3 text-white focus:border-red-500 outline-none mono text-xs uppercase"
                    value={newAdminEmail}
            ...
            <div className="space-y-2">
                <div className="p-3 bg-black/40 border border-zinc-800 flex justify-between items-center">
                    ...
                    <span className="text-[9px] mono text-red-500 uppercase font-black">Master Key</span>
                </div>
                {admins.filter(a => a !== 'bradleyjsmithuk@gmail.com').map(email => (
                    <div key={email} className="p-3 bg-black/20 border border-zinc-800 flex justify-between items-center group">
                        ...
                        <button 
                          ...
                          className="text-[9px] mono text-zinc-600 hover:text-red-500 uppercase font-black opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
                        >
                            REVOKE ACCESS
                        </button>
                    </div>
      ...
      {showAdd && (
        <form onSubmit={createBattle} className="p-8 space-y-6 mb-8 bg-zinc-900 border border-zinc-700 relative shadow-2xl">
          ...
          <h3 className="text-2xl font-black uppercase italic tracking-tighter">Initialize Battle Sequence</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              ...
              <input 
                required
                className="w-full bg-black border border-zinc-800 p-3 text-white focus:border-red-500 outline-none mono text-sm"
            ...
            <div>
              <label className="block text-[10px] uppercase mb-2 text-red-500/70 font-black tracking-widest">Entry A Name</label>
              <input 
                required
                className="w-full bg-black border border-zinc-800 p-3 text-white focus:border-red-500 outline-none mono text-sm"
            ...
            <div>
              <label className="block text-[10px] uppercase mb-2 text-blue-500/70 font-black tracking-widest">Entry B Name</label>
              <input 
                required
                className="w-full bg-black border border-zinc-800 p-3 text-white focus:border-blue-500 outline-none mono text-sm"
                placeholder="ARTIST NAME"
          ...
          <div className="flex items-center gap-3 bg-black/40 p-4 border border-zinc-800">
            ...
            <label htmlFor="startNow" className="text-xs font-black uppercase italic tracking-tight cursor-pointer">
              Deploy as ACTIVE (Allow votes immediately)
            </label>
          </div>

          <button 
            type="submit"
            className="w-full bg-red-600 text-white font-black uppercase py-5 text-xl skew-x-[-3deg] hover:bg-red-500 shadow-[0_6px_0_#991b1b] active:translate-y-1 active:shadow-none transition-all"
          >
            DEPLOY TO ARENA GATE
          </button>
        </form>
      )}
          ...
          <div key={battle.id} className="p-5 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 transition-all">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className={`px-3 py-1 text-[10px] font-black uppercase border-2 flex items-center gap-2 ${
                battle.status === 'active' ? 'bg-red-600/20 border-red-500 text-red-500 animate-pulse' :
                battle.status === 'finished' ? 'bg-zinc-800 border-zinc-700 text-zinc-400' :
                'bg-zinc-800 border-zinc-700 text-zinc-500'
              }`}>
                {battle.status === 'active' && <span className="w-1.5 h-1.5 bg-red-500 rounded-full"></span>}
                {battle.status}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-px bg-zinc-800 border border-zinc-800 mb-6 rounded overflow-hidden">
              <div className="bg-black/40 p-4">
                <span className="mono text-[9px] block text-red-500/70 font-black uppercase mb-1 tracking-widest">LOAD A: {battle.artistA}</span>
                <span className="text-3xl font-black">{battle.votesA}</span>
              </div>
              <div className="bg-black/40 p-4">
                <span className="mono text-[9px] block text-blue-500/70 font-black uppercase mb-1 tracking-widest">LOAD B: {battle.artistB}</span>
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-zinc-800/50">
              {battle.status === 'pending' && (
                <button 
                  ...
                  className="bg-red-600 text-white px-4 py-1.5 text-[10px] font-black uppercase flex items-center gap-2 hover:bg-red-500 transition-colors"
                >
                  <Play size={10} fill="currentColor" /> Open Floor
                </button>
              )}
              ...
              {battle.status === 'finished' && (
               ...
               <button 
                  ...
                  className="ml-auto mono text-[9px] uppercase text-zinc-600 hover:text-red-500 transition-colors py-1 px-2"
                >
                  Clear Feed
                </button>
            </div>
    ...
    <div className="min-h-[70vh] grid grid-cols-1 lg:grid-cols-2 gap-12 items-center animate-in fade-in duration-1000">
        ...
        <AnimatePresence mode="wait">
            {activeBattle.status === 'active' ? (
                <motion.div 
                    ...
                    exit={{ y: 20, opacity: 0 }}
                    className="inline-block bg-red-600 text-white px-12 py-3 text-3xl font-black uppercase italic tracking-tighter shadow-[0_0_40px_rgba(220,38,38,0.4)]"
                >
                    Voting Results
                </motion.div>
            ) : (
                <motion.div 
                    ...
                    animate={{ y: 0, opacity: 1 }}
                    exit={{ y: 20, opacity: 0 }}
                    className="inline-block bg-zinc-100 text-black px-12 py-3 text-3xl font-black uppercase italic tracking-tighter shadow-[0_0_40px_rgba(255,255,255,0.2)]"
                >
                    Voting Results
                </motion.div>
            )}
      ...
      <div className="space-y-8 relative">
        ...
        <div className="space-y-2 relative z-10">
          <div className="flex justify-between items-end">
            ...
            <span className="text-6xl md:text-8xl font-black text-red-500 tracking-tighter italic">{percentA}%</span>
          </div>
          <div className="h-16 md:h-24 w-full bg-zinc-900 border-2 border-zinc-800 relative group overflow-hidden">
             <motion.div 
               initial={{ width: 0 }}
               animate={{ width: `${percentA}%` }}
               className="absolute h-full bg-red-600 shadow-[0_0_30px_#ef4444]"
             />
      ...
      <div className="space-y-8 relative text-right">
        ...
        <div className="space-y-2 relative z-10">
          <div className="flex justify-between items-end flex-row-reverse">
             ...
             <span className="text-6xl md:text-8xl font-black text-blue-500 tracking-tighter italic">{percentB}%</span>
          </div>
          <div className="h-16 md:h-24 w-full bg-zinc-900 border-2 border-zinc-800 relative group overflow-hidden">
             <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${percentB}%` }}
                className="absolute h-full bg-blue-600 shadow-[0_0_30px_#3b82f6] right-0"
             />
      ...
      <div className="lg:col-span-2 flex justify-center pt-12">
        <div className="bg-zinc-900/50 border border-zinc-800 px-12 py-4 flex items-center gap-6 rounded-full backdrop-blur-md">
            ...
            <span className="mono text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500">Live Audience Participation Feed</span>
        </div>
      </div>
    </div>
  );
